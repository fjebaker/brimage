...
double calc_diff(const Canvas& reference, const Canvas& canvas) {
...
void random_walk(const Canvas& reference, Canvas& canvas) {
    ...
    SubCanvas mypart(sqwidth, sqwidth);
    ...
    for (int i = 0; i < SEGMENT_NO; i++) {
        ...
        // get subregions
        reference.subregion(impart,
            corx, corx+sqwidth, cory, cory+sqwidth
        );
        canvas.subregion(mypart,
            corx, corx+sqwidth, cory, cory+sqwidth
        );

        localerr = calc_diff(impart, mypart);
        ...
        for (int a = 0; a < ANGLES_NO; a++) {
            ...
            if (canvas.in_bounds(nx, ny)) {
                ...
                l.trace(mypart, shade);

                currerr = calc_diff(impart, mypart);
